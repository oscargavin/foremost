 ...
 *
 * Concept: A welcoming network that invites connection,
 * representing the start of a conversation.
 *
...
function MobileAnimation() {
  ...
  const initNetwork = useCallback((width: number, height: number) => {
    ...
    const nodePositions = [
      { x: 0.12, y: 0.10, type: "tertiary" as const, radius: 1.5 },
      { x: 0.88, y: 0.08, type: "secondary" as const, radius: 2.5 },
      { x: 0.92, y: 0.30, type: "tertiary" as const, radius: 2 },
      { x: 0.85, y: 0.50, type: "primary" as const, radius: 4 },
      { x: 0.95, y: 0.68, type: "tertiary" as const, radius: 1.5 },
      { x: 0.80, y: 0.88, type: "secondary" as const, radius: 2.5 },
      { x: 0.15, y: 0.95, type: "tertiary" as const, radius: 2 },
    ];
...
function DesktopAnimation() {
  ...
  const initNetwork = useCallback((width: number, height: number) => {
    ...
    // Primary nodes in a welcoming cluster on the right side
  ...
  const draw = useCallback(
    (ctx: CanvasRenderingContext2D, width: number, height: number) => {
      ...
      ctx.save();
      ctx.translate(width - 65, height - 65);
