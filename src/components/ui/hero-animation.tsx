 ...
 *
 * Desktop: Interactive network that responds to mouse - "Your attention brings clarity"
 * Mobile: Ambient constellation that breathes quietly - "Intelligence at work"
 *
 * Both versions use the same visual language but different interaction models
 * appropriate to their input methods and screen contexts.
 */
...
// ============================================================================
// MOBILE VERSION - Ambient, passive, subtle
// ============================================================================
...
function MobileAnimation() {
  ...
  const initNetwork = useCallback((width: number, height: number) => {
    ...
    // Mobile: Sparse constellation spread across the viewport
    // Positioned to frame content rather than compete with it
    const nodePositions = [
      // Top area - subtle presence above content
    ...
    // Create sparse connections - only between nearby nodes
    const connectionPairs = [
      [1, 3], // top-right to mid-right primary
      [2, 3], // corner to primary
      [3, 4], // primary to below
      [4, 5], // continuing down right side
      [5, 7], // bottom right to bottom center
    ];
  ...
  const draw = useCallback((ctx: CanvasRenderingContext2D, width: number, height: number) => {
    ...
    // Update node positions - gentle floating only
    nodes.forEach((node) => {
      // Very subtle floating motion - slower and smaller amplitude
      ...
      // Ambient activation - slow wave that passes through the network
      const wavePhase = slowTime * 0.0003 + node.pulsePhase * 0.5;
      const waveActivation = (Math.sin(wavePhase) + 1) * 0.15; // 0 to 0.3
      node.activated = waveActivation;
    });

    // Update connections based on connected node activation
    ...
    // Occasionally spawn ambient particles (less frequent than desktop)
    if (time % 120 === 0 && particles.length < 5) {
      ...
      if (activeConns.length > 0) {
        ...
        particles.push({
          ...
          progress: 0,
          speed: 0.003 + Math.random() * 0.004, // Slower particles
    ...
    for (let i = particles.length - 1; i >= 0; i--) {
      ...
      const alpha = Math.sin(p.progress * Math.PI) * 0.5; // Lower max opacity
    ...
    nodes.forEach((node) => {
      ...
      // Node fill - lower opacity than desktop
      ctx.beginPath();
...
// ============================================================================
// DESKTOP VERSION - Interactive, responsive to attention
// ============================================================================
...
function DesktopAnimation() {
  ...
  const initNetwork = useCallback((width: number, height: number) => {
    ...
    // Primary nodes - positioned in a loose diamond pattern on the right side
    ...
    primaryPositions.forEach((pos) => {
      nodes.push({
        x: width * pos.x + (Math.random() - 0.5) * 40,
        y: height * pos.y + (Math.random() - 0.5) * 40,
        baseX: width * pos.x,
        baseY: height * pos.y,
        vx: 0,
        vy: 0,
    ...
    // Tertiary nodes - scattered around the network
    for (let i = 0; i < 16; i++) {
